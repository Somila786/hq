...
//
// Built: 2026-08-31
// Source: src/auth.js + src/import.js + src/db.js + src/views.js + src/index.js
 ...
 */
function parseLeads(text, { source = "apify" } = {}) {
 ...
 * every one of them.
 */
function classifyLeads(rows, { existingEmails = new Set(), existingKeys = new Set(), skipNoEmail = true } = {}) {
...
async function getClientsForOwner(env, userId) {
...
async function getLeadsForOwner(env, userId) {
  const { results } = await env.DB.prepare(
    `SELECT id, name, company, contact_email, stage, value_estimate, source, owner, owner_user_id, notes,
            created_at, updated_at,
            COALESCE(outreach_status,'pending') AS outreach_status,
...
async function assignLeadOwner(env, id, ownerUserId) {
...
async function getLeadDedupeKeys(env) {
  const { results } = await env.DB.prepare(
...
async function getCallQueueForOwner(env, userId, limit = 200) {
  const { results } = await env.DB.prepare(
    `SELECT l.id, l.name, l.company, l.contact_email, l.stage, l.owner, l.source,
            l.value_estimate, l.outreach_last_sent_at, l.call_due_at,
            l.call_due_at <= datetime('now') AS due_now,
...
async function countCallQueueForOwner(env, userId) {
  const row = await env.DB.prepare(
    `SELECT
       SUM(call_due_at <= datetime('now')) AS due,
       SUM(call_due_at >  datetime('now')) AS waiting
...
async function getCallOutcomeStatsForOwner(env, userId) {
  const { results } = await env.DB.prepare(
    `SELECT call_outcome AS outcome, COUNT(*) AS n
...
return { getFreelancers, getFreelancerById, createFreelancer, setFreelancerActive, getClients, createClient, getClientsForOwner, setClientStatus, getLeads, createLead, getLeadsForOwner, assignLeadOwner, listCoordinators, getLeadDedupeKeys, setLeadDraft, setLeadValue, appendLeadNotes, updateLeadStage, getRevenueEntries, createRevenueEntry, getWeeklyEntry, upsertWeeklyEntry, getFreelancerHistory, getDashboard, replaceBackupCodes, countUnusedBackupCodes, redeemBackupCode, clearBackupCodes, listUsers, getUserById, getFreelancersWithoutUser, createUser, setUserTitle, reissueSetupToken, revokeUserAccess, countActiveFounders, createInviteCode, findOpenInviteCode, consumeInviteCode, listInviteCodes, revokeInviteCode, countOpenInviteCodes, getUserByEmail, getUserCredentials, claimSubmission, purgeOldSubmissions, bindGoogleSub, registerOAuthClient, getOAuthClient, createAuthCode, consumeAuthCode, storeMcpToken, getMcpTokenUser, consumeRefreshToken, listMcpGrants, revokeMcpGrant, revokeAllMcpTokensForUser, purgeExpiredOAuth, recordOutreachEvent, findLeadByEmail, getLeadById, getOutreachForLead, getRecentOutreach, getOutreachSummary, getUnmatchedOutreach, setOutreachStatus, markOutreachSent, openCallWindowFromEvent, getCallQueue, countCallQueue, getCallOutcomeStats, getCallQueueForOwner, countCallQueueForOwner, getCallOutcomeStatsForOwner, logCallOutcome, reopenCallWindow, getLeadsAwaitingApproval, countOutreachQueue, logAudit, getAuditLog, logError, getErrorLog, flagForRetentionReview, getOpenRetentionFlags, resolveRetentionFlag, eraseLeadPII, eraseFreelancerPII, runRetentionScan, CALL_OUTCOMES, CALL_OUTCOME_LABELS };
})();
...
function leadDetailPage({ user, lead, events, theme, webhookReady, csrf, sendingReady = false, greetingNow = "Good day", styleWarnings = [], coordinators = [] }) {
...
function leadImportPage({ user, csrf, theme, error = null, raw = "", source = "apify", skipNoEmail = true }) {
  return layout({
    user,
    active: "leads",
    title: "Import leads",
...
function leadImportPreviewPage({ user, csrf, theme, rows, counts, raw, source, skipNoEmail }) {
